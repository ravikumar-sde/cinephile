...
const MovieGrid = ({ movies, loading, mediaType = 'movie' }) => {
  ...
  if (loading) {
    return (
      <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5">
        {[...Array(18)].map((_, i) => (
    ...
    return (
      <div className="flex-1 flex items-center justify-center py-16">
        <div className="text-center text-gray-500">
          <i className='bx bx-movie text-6xl mb-4'></i>
  ...
  return (
    <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5">
      {movies.map((item) => (
        ...
        >
          <div className="relative aspect-[2/3] rounded-lg overflow-hidden mb-2 shadow-lg">
            {/* Rating Badge */}
            <div className={`absolute top-2 left-2 z-10 w-9 h-9 rounded-full ${getRatingColor(item.vote_average)} flex items-center justify-center shadow-lg border-2 border-gray-900`}>
            ...
            {item.poster_path ? (
              ...
              <div className="w-full h-full bg-gray-800 flex items-center justify-center">
                <i className='bx bx-movie text-4xl text-gray-600'></i>
              </div>
            )}
          ...
          {/* Title */}
          <h3 className="text-gray-200 font-semibold text-sm leading-tight line-clamp-2 group-hover:text-red-500 transition-colors">
